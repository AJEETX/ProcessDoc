	...
	public class ErrorLog
	{
        ...
        public static bool ErrorRoutine(bool bLogType,Exception  objException)
        {
	        try
	        {
                ...
                bLoggingEnabled = CheckLoggingEnabled(bLogType);
        ...
        /// <summary>
        /// Check Login status config file is exist. If exist read the value set the loggig status
        /// </summary>
        public static bool CheckLoggingEnabled(bool log)
        {
	        ...
	        {
                return (GetStatus(log)== "True");	
		        //Read the value from xml and set the logging status
                //bool bTemp	= GetValueFromXml(strLoggingStatusConfig,"Custom");
                //return bTemp;
	        }
        }
        /// <summary>
        /// Check the Login status config under debug or release folder. If not exist, check under 
        ...
        {
	        string strCheckinBaseDirecotry	= AppDomain.CurrentDomain.BaseDirectory+ "App.Config";

	        if (File.Exists(strCheckinBaseDirecotry))
		        return strCheckinBaseDirecotry;
	        else
	        {
		        string strCheckinApplicationDirecotry	=	GetApplicationPath() + "App.Config";
        ...
        /// <returns></returns>
        /// 
        private static string GetStatus(bool loggingType)
        {
            string Log = "CustomLog";
            Configuration configuration = ConfigurationManager.OpenExeConfiguration(ConfigurationUserLevel.None);
            if (!loggingType)
                Log = ConfigurationManager.AppSettings[Log].ToString();
            else
            {
                Log = "EventLog";
                Log = ConfigurationManager.AppSettings[Log].ToString();
        ...
        //Not being used
        private static bool GetValueFromXml(string strXmlPath,string Log)
        {
	        try
	        {
                ...
                //string x = contactDoc.SelectSingleNode("configuration/appSettings/add/@key").Value;
		        //Get a node
                XmlNodeList UserList = contactDoc.GetElementsByTagName("add");
                for (int i = 0; i < UserList.Count; i++)
                {
                    attrKey = UserList[i].Attributes["key"].Value;
                    if (Log+"Log"==attrKey)
                    {
                        attrVal = UserList[i].Attributes["Value"].Value;
                    }
                }  
		        //get the value
